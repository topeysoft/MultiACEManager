...
READER_POLL_INTERVAL = 2.0  # Increased from 0.5s to prevent UI state flickering
WRITER_POLL_INTERVAL = 2.0  # Poll device status every 2 seconds
SENSOR_POLL_INTERVAL = 2.0  # Increased from 0.5s to prevent UI state flickering
CONNECTION_DEBOUNCE_TIME = 6.0  # Seconds of stability required before reporting state change to UI (2x poll interval)
REQUEST_TIMEOUT = 5.0  # Must be greater than WRITER_POLL_INTERVAL to prevent timeout loops
